...
export const useDeck = create<DeckState>((set, get) => ({
  ...
  init: async () => {
    set({ loading: true });
    const [cards, srs, reviews] = await Promise.all([
      db.cards.toArray(),
      db.srs.toArray(),
      db.reviews.toArray()
    ]);
    ...
    set({ cards, srs, reviews, queue, loading: false, ready: true, todayNew: 0, todayReview: 0, todayCorrect: 0 });
    get().nextCard();
  },
  refreshQueue: () => {
    const { cards, srs } = get();
  ...
  grade: async (grade) => {
    ...
    await db.transaction('rw', db.cards, db.srs, db.reviews, async () => {
  ...
  exportProgressJson: async () => {
    const [cards, srs, prefs, reviews] = await Promise.all([
      db.cards.toArray(),
      db.srs.toArray(),
      db.prefs.toArray(),
      db.reviews.toArray()
    ...
    const payload = {
      cards,
      srs,
      prefs: prefs[0] ?? usePrefs.getState().prefs,
      reviews
    };
